...
import { ViewBayiModel } from '~/app/models/bayi.model';
...
@Component({
    moduleId: module.id,
    selector: 'ns-hitung-stunting-beranda',
    templateUrl: './hitung-stunting-beranda.component.html',
})

export class HitungStuntingBerandaComponent implements OnInit {

    DataBalita: ValueList<string>;
    ...
    JenisKelaminDropdownValue: number = 0;
    ...
    DatepickerTglLahirValue: Date = new Date();

    SwitchAsiValue: boolean = true;
        ...
        this.bayiService.onGetBayiByIdPosyandu(id_posyandu)
            .subscribe((result) => {
                ...
                result.data.forEach((item) => {
                    itemSource.push({
                        value: item.id, display: item.nama_bayi
                    });
                });

                this.DataBalita = new ValueList<string>(itemSource);

                this.DataJenisKelamin = new ValueList<string>(this.DataJenisKelamin$);
            });
    }

    onChangeBalita(args: SelectedIndexChangedEventData): void {
        let index = args.newIndex;

        let selectedBalita = this.DataBalita$[index];

        this.id_bayi.setValue(selectedBalita.id);
        this.jenis_kelamin.setValue(selectedBalita.jenis_kelamin);
        this.JenisKelaminDropdownValue = this.DataJenisKelamin$.findIndex((item) => { return item.value == selectedBalita.jenis_kelamin });

        this.asi.setValue(selectedBalita.asi);
        this.SwitchAsiValue = selectedBalita.asi == 1 ? true : false;
        this.DatepickerTglLahirValue = new Date(selectedBalita.tanggal_lahir);
    }
    ...
    onChangeJenisKelamin(args: SelectedIndexChangedEventData): void {
        ...
        this.JenisKelaminDropdownValue = index;
    }
